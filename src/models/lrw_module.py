...
from src.utils.mixup import mixup_criterion, mixup_data
...
class LRWLitModule(LightningModule):
    ...
    def __init__(
        ...
        if self.hparams.label_smooth:
            ...
            self.criterion = torch.nn.CrossEntropyLoss()  # default reducrion is "mean"
    ...
    def step(self, batch: Any):
        ...
        if self.hparams.use_mixup:
            if self.hparams.use_boundary:
                x, border, y_a, y_b, lam = mixup_data(x, y, border, alpha=1.0)
            else:
                x, border, y_a, y_b, lam = mixup_data(x, y, alpha=1.0)
